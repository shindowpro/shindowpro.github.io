...
if (myCursor.length && $("body")) {
    ...
    window.onmousemove = function(i) {
        t || (e.style.transform = "translate(" + i.clientX + "px, " + i.clientY + "px)"),
    ...
    $("body").on("mouseleave", "a,.trigger, .cursor-pointer", function() {
        $(this).is("a") && $(this).closest(".cursor-pointer").length || (r.classList.remove("cursor-hover"),
...
button.on("click", function() {
    var r = jQuery(this);
    return r.hasClass("showme") || (button.removeClass("showme"),
    r.addClass("showme")),
    !1
}),
